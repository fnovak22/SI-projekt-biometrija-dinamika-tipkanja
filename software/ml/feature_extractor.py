...
def _features_from_key_events(keydowns, keyups, all_events=None):
    ...
    if len(sorted_events) >= 2:
        duration_ms = round(_event_time(sorted_events[-1]) - _event_time(sorted_events[0]), 3)
    ...
    return {
        ...
        "avg_dd_interval_ms": _avg(dd_intervals) if dd_intervals else None,
        "std_dwell_ms": _std(dwell_times),
        "std_dd_interval_ms": _std(dd_intervals),
